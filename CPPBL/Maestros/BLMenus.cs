namespace CPPBL.Maestros
{
    ...
    public class BLMenus
    {
        ...
        public List<EntitiesMenus> Consultarmenus(DLAccesEntities parametrosConsulta)
        {
            List<EntitiesMenus> lstmenu = new List<EntitiesMenus>();
            DataTable menuTable = this.acceso.Consultas(parametrosConsulta);
            foreach (DataRow fila in menuTable.Rows)
            {
                EntitiesMenus menuItem = new EntitiesMenus();
                menuItem.cppMn_IdMenu = Convert.ToInt32(fila["cppMn_IdMenu"].ToString());
                menuItem.cppMn_Texto = fila["cppMn_Texto"].ToString();
                menuItem.cppMn_Descripcion = fila["cppMn_Descripcion"].ToString();
                menuItem.cppMn_idPadre = Convert.ToInt32(fila["cppMn_idPadre"].ToString());
                menuItem.cppMn_url = fila["cppMn_url"].ToString();
                menuItem.cppMn_Activo = Convert.ToBoolean(fila["cppMn_Activo"].ToString());
